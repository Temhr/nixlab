{...}: {
  flake.nixosModules.hosts--c-optional--virtualizations = {
    config,
    lib,
    pkgs,
    ...
    ...
    config = lib.mkMerge [
      ...
      (lib.mkIf config.incus.enable {
        ...
        networking = {
          ...
          nat = {
            ...
            forwardPorts = [
              {
                destination = "10.24.51.164:8123";
                proto = "tcp";
                sourcePort = 8123;
              }
              {
                destination = "10.24.51.164:1880";
                proto = "tcp";
                sourcePort = 1880;
              }
              {
                destination = "10.24.51.164:2665";
                proto = "tcp";
                sourcePort = 2665;
              }
            ];
      ...
      (lib.mkIf config.quickemu.enable {
        environment.systemPackages = with pkgs; [unstable.quickemu unstable.quickgui unstable.spice-gtk]; #Quickly create and run optimised Windows, macOS and Linux virtual machines
      })
