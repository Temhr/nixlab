{ config, lib, pkgs, ... }: {
    ...
    config = lib.mkMerge [
        ...
        (lib.mkIf config.ghostty.enable {
          environment.systemPackages = [ ghostty.packages.x86_64-linux.default ]; #Fast, feature-rich terminal emulator that uses platform-native UI and GPU acceleration
        })
