  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.podman-bookstack = {
      ...
      preStart = lib.mkBefore ''
        echo "APP_KEY=$(cat ${cfg.appKeyFile})" > /run/bookstack-appkey.env
        chmod 600 /run/bookstack-appkey.env
      '';
    };
    ...
    virtualisation.oci-containers.containers.bookstack = {
      ...
      # dbPasswordFile contains: DB_PASS=value  (already KEY=value format)
      # /run/bookstack-appkey.env is generated by the preStart script below
      environmentFiles = [
        cfg.dbPasswordFile
        /run/bookstack-appkey.env
      ];
