...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    # sops-nix decrypts to bare values so preStart wraps them into KEY=value format
    # ----------------------------------------------------------------------------
    virtualisation.oci-containers.containers.bookstack-db = {
      ...
      environment = {
        ...
        # Passwords are NOT set here — injected via /run/bookstack-db.env below
      };

      # Generated by the preStart script in podman-bookstack-db service override
    ...
    # Wait for directories, then build the env file from bare sops secret values
    ...
    # Wrap the bare APP_KEY secret value into KEY=value format before the
    # container starts, since sops-nix decrypts to bare values not KEY=value lines
    ...
    # Passwords injected via environmentFiles — never stored in the Nix store
    # APP_KEY is a bare value from sops-nix so a preStart script wraps it into
    # KEY=value format that environmentFiles expects, written to /run/bookstack-appkey.env
    # ----------------------------------------------------------------------------
    virtualisation.oci-containers.containers.bookstack = {
      ...
      environment = {
        ...
        # container — works for both rootful and rootless Podman. MariaDB is on
        # the host network (--network=host) so this reaches it correctly.
        DB_HOST     = "host.containers.internal";
        DB_PORT     = "3306";
        DB_USER     = "bookstack";
        DB_DATABASE = "bookstack";
        # DB_PASS and APP_KEY are NOT set here — injected via environmentFiles
      };

      # Both DB_PASS and APP_KEY are bare values from sops-nix —
      # the preStart script below wraps them into KEY=value format
