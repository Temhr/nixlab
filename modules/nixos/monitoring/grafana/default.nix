        ...
        serviceConfig =
          {
            ...
            ExecStartPre = [
              ...
              ("+${pkgs.writeShellScript "grafana-setup" ''
                # ── known dashboard names (baked in at eval time) ─────────────
                known="${lib.concatStringsSep " " (builtins.attrNames allDashboards)}"

                # Helper: returns 0 if $1 is in $known, 1 otherwise.
                is_known() {
                  for k in $known; do
                    [ "$k" = "$1" ] && return 0
                  done
                ...
                # ── remove stale dashboard directories ────────────────────────
                if [ -d ${cfg.dataDir}/dashboards ]; then
                  for dir in ${cfg.dataDir}/dashboards/*/; do
                    [ -d "$dir" ] || continue
                    name="$(basename "$dir")"
                    if ! is_known "$name"; then
                      echo "grafana-setup: removing stale dashboard dir: $name"
                      rm -rf "$dir"
                    fi
                  done
                fi

                # ── remove stale provisioning YAML configs ────────────────────
                ...
                # ── remove stale records from the Grafana SQLite database ─────
                db="${cfg.dataDir}/data/grafana.db"
                if [ -f "$db" ]; then
                  # dashboard_provisioning.name matches the provider name we set
                  # in the YAML (same as the dashboard key in allDashboards).
                  # Any provisioned dashboard whose provider name is no longer
                  # in the known set is stale and safe to delete.
                  known_sql="${lib.concatStringsSep ","
                    (map (n: "'${n}'") (builtins.attrNames allDashboards))}"

                  stale_uids=$(${pkgs.sqlite}/bin/sqlite3 "$db" \
                    "SELECT d.uid
                     FROM dashboard d
                     INNER JOIN dashboard_provisioning dp ON dp.dashboard_id = d.id
                     WHERE dp.name NOT IN ($known_sql);")

                  if [ -n "$stale_uids" ]; then
                    for uid in $stale_uids; do
                      echo "grafana-setup: removing stale DB record uid=$uid"
                      ${pkgs.sqlite}/bin/sqlite3 "$db" \
                        "DELETE FROM dashboard WHERE uid = '$uid';"
                    done
                  fi

                  # Remove orphaned folder records — folders that have no
                  # remaining child dashboards and no provisioning entry.
                  ${pkgs.sqlite}/bin/sqlite3 "$db" \
                  ...
                  echo "grafana-setup: cleaned up orphaned folder records"
                fi
